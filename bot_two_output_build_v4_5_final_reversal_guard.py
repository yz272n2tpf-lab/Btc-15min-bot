...
# These are ENTRY guidance only — never promoted to FINAL CALL authority.
if not _fair_ready:
...
elif _opportunity_model_prob >= 0.70 and _opportunity_edge >= 0.15:
    _opportunity_status = 'OPPORTUNITY'
    _opportunity_reason = 'Target-aware fair >=70% with >=15pt edge at <=50c'
elif _opportunity_model_prob >= 0.60 and _opportunity_edge >= 0.08:
    _opportunity_status = 'VALUE WATCH'
    _opportunity_reason = 'Early value is forming; fair >=60% with >=8pt edge at <=50c'
else:
    _opportunity_status = 'WATCH'
    _opportunity_reason = 'Price may be cheap, but target-aware evidence/edge is not strong enough yet'
...
try:
    ...
    # V4.5 reversal guard:
    # When a lot of time remains, require more strike separation.
    # Evidence from the short live tests:
    # - the failed 8.79m-left call was only ~$94 from strike
    # - prior successful >7m-left calls were >= ~$106 from strike
    # Keep the original $75 threshold once <=7 minutes remain.
    _decisive_required_gap = (
        100.0 if _decisive_time_left is not None and _decisive_time_left > 7.0
        else 75.0
    )
...
if _final_15m_ready:
    ...
    if _decisive_final_ready:
        ...
        _two_final_reason = (
            "Decisive target-aware fair gate with reversal guard: "
            "fair >=92%, <=10m left, gap >=$100 when >7m left "
            "or >=$75 when <=7m left"
        )
...
print(
    "DECISIVE REQUIRED GAP:",
    f"${_decisive_required_gap:.0f}" if '_decisive_required_gap' in globals() else "N/A"
)
...
# ============================================================
_snapshot_log = Path('kalshi_two_output_live_log_v4_5.csv')
_snapshot_fields = [
    ...
    'opportunity_status','price_zone',
    'ladder_status','ladder_direction','ladder_confidence','checkpoints',
    'expected_final_outcome','final_status','final_side','final_confidence','final_call_source','decisive_fair_ready','decisive_required_gap',
...
_snapshot_row = {
    ...
    'decisive_required_gap': _decisive_required_gap if '_decisive_required_gap' in globals() else None,
