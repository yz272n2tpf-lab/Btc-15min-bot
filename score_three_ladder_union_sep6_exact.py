...
for r in read(UFILE):
    ...
    if t and START<=t<=END:u.append(r)
...
for r in u:
    ...
    if (math.isfinite(pf) and pf>=.90 and math.isfinite(ml) and ml<=8 and math.isfinite(gap)
        and abs(gap)>=need and math.isfinite(ratio) and ratio>=1.0 and ps and ps==target):
...
for r in u:
    ...
    ask=prob(r.get("side_ask")); pf=prob(r.get("preferred_fair")); edge=num(r.get("preferred_edge"))
    if math.isfinite(edge) and abs(edge)>1.5: edge/=100.0
    ml=num(r.get("minutes_left")); gap=num(r.get("btc_gap"))
    if (math.isfinite(ask) and ask<=.45 and math.isfinite(pf) and pf>=.75 and math.isfinite(edge)
        and edge>=.08 and math.isfinite(ml) and 2<=ml<=10 and math.isfinite(gap) and abs(gap)>=25):
...
for r in fresh_scalp:
    ...
    if c not in S:
        ...
        if not math.isfinite(ml):
            sec=num(r.get("seconds_left")); ml=sec/60.0 if math.isfinite(sec) else math.nan
        S[c]=(t,ml)

f,e,s=set(F),set(E),set(S)
union=f|e|s; dead=set(contracts)-union
pct=lambda a,b:100*a/b if b else 0.0
overall_scalp_contracts={str(r.get("contract") or r.get("ticker") or "").strip() for r in fresh_scalp if str(r.get("contract") or r.get("ticker") or "").strip()}

print("="*78)
print("SEP 6 — EXACT THREE-LADDER UNION TEST")
print("="*78)
...
print(f"Fresh scalp rows:  {len(fresh_scalp)}")
print(f"Fresh scalp unique contracts overall: {len(overall_scalp_contracts)}")
print("-"*78)
...
for k in ["EARLY","SCALP","FINAL","UNKNOWN"]: print(f"{k:10s}: {first[k]}")
print()
