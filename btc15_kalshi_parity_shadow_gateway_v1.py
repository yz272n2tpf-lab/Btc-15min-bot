#!/usr/bin/env python3
"""
BTC15 KALSHI/BRTI PARITY SHADOW V3 — GATEWAY BRTI SHADOW
...
READ-ONLY. BRTI FROM ONE-OWNER GATEWAY. NO CORE LOGIC CHANGES. NO TRADING THRESHOLD CHANGES. NO ORDERS.
"""
...
def direct_brti_payload():
    """Gateway-backed BRTI payload; zero direct CF HTTP. Fail closed."""
    x = gateway_brti_state()
    return [(datetime.fromtimestamp(int(x["source_ts_ms"])/1000.0,tz=timezone.utc),float(x["value"]))]
def market_target(m):
