...
def main():
    if not PROCESSED_CACHE.exists():
        raise SystemExit("MISSING processed cache: "+str(PROCESSED_CACHE))
    if not CANDLE_CACHE.exists():
        raise SystemExit("MISSING candle cache: "+str(CANDLE_CACHE))

    d=pd.read_csv(PROCESSED_CACHE)
    ...
    ticks["start"]=ticks["ticker"].map(lambda t: parse_contract_times(t)[0])
