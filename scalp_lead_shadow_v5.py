#!/usr/bin/env python3
"""V5 research-only scalp lead-time collector. Signal-only. NO ORDERS.

Extends V4 with two safeguards aimed at the false positives seen in live shadow:
(1) impulse persistence across consecutive seconds, and (2) broader 30s structure
that must not be materially fighting the proposed scalp side. V4 remains a
baseline; V5 is a parallel label only.
"""
...
POLL=1.0; KEEP=240; HORIZON=90; MAX_ASK=.45
...
V5_BTC30_FLOOR=-5.0; V5_BRTI15_FLOOR=0.0; V5_CONFIRM_WINDOW=4.0; V5_CONFIRM_COUNT=2
...
hist=deque(); pending=[]; last={}; proto=defaultdict(deque)
...
def features(row,side):
    ...
    b5,b15,b30=d(row,side,'btc',5),d(row,side,'btc',15),d(row,side,'btc',30)
    a5,a15=d(row,side,'ask',5),d(row,side,'ask',15)
    r5,r15=d(row,side,'brti',5),d(row,side,'brti',15)
    if None in (b5,b15,a5,a15):return None
    accel=b5-(b15/3.0)
    broad=(b5>=BTC5_MIN or b15>=BTC15_MIN) and (r5 is None or r5>0) and a5<=MAX_ASK5
    v4=(broad and row['left']>=V4_MIN_LEFT and ask>=V4_MIN_ASK and b5>=V4_BTC5 and b15>=V4_BTC15 and (r5 is not None and r5>=V4_BRTI5) and accel>=V4_MIN_ACCEL and a5<=V4_MAX_ASK5 and a15<=V4_MAX_ASK15)
    structure_ok=(b30 is None or b30>=V5_BTC30_FLOOR) and (r15 is None or r15>=V5_BRTI15_FLOOR)
    return {'btc5':b5,'btc15':b15,'btc30':b30,'brti5':r5,'brti15':r15,'ask5':a5,'ask15':a15,'accel':accel,'broad':broad,'v4':v4,'structure_ok':structure_ok}

def proto_confirm(row,side,f):
    k=(row['ticker'],side); q=proto[k]
    now=row['ts']
    while q and q[0] < now-V5_CONFIRM_WINDOW:q.popleft()
    if f['v4'] and f['structure_ok']:q.append(now)
    return len(q)>=V5_CONFIRM_COUNT
...
def add(row,side,f,grade):
    ...
    print('LEAD_V5 CANDIDATE | %s | %s | %s | ask %.3f | btc5 %+.2f | btc15 %+.2f | btc30 %s | accel %+.2f | brti5 %s | brti15 %s | ask5 %+.3f | ask15 %+.3f | left %.0fs'%(grade,row['ticker'],side,ask,f['btc5'],f['btc15'],fmt(f['btc30']),f['accel'],fmt(f['brti5']),fmt(f['brti15']),f['ask5'],f['ask15'],row['left']),flush=True)
...
def resolve(row):
    ...
    for e in done:
        ...
        print('LEAD_V5 RESULT | %s | %s | %s | entry %.3f | max_exec_gain %+.3f | adverse %+.3f | hit10 %s | to_exec+5c %s | to_exec+10c %s | kalshi_reprice+5c %s'%(e['grade'],e['ticker'],e['side'],e['entry'],e['max']-e['entry'],e['min']-e['entry'],e['x10'] is not None,t5,t10,ar),flush=True)

print('SCALP LEAD SHADOW V5 START | V4 baseline + persistence/structure filter | NO ORDERS',flush=True)
while True:
    t=time.time()
    try:
        row=snap()
        if row:
            ...
            for side in ('UP','DOWN'):
                f=features(row,side)
                if not f or not f['broad']:continue
                if f['v4']:add(row,side,f,'V4_BASELINE')
