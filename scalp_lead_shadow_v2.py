#!/usr/bin/env python3
"""Independent signal-only scalp lead-time collector. NO ORDERS."""
...
def candidate(row,side):
    ...
    b5,b15,a5=d(row,side,'btc',5),d(row,side,'btc',15),d(row,side,'ask',5); r5=d(row,side,'brti',5)
    if None in (b5,b15,a5):return None
    if (b5>=BTC5_MIN or b15>=BTC15_MIN) and (r5 is None or r5>0) and a5<=MAX_ASK5:return {'btc5':b5,'btc15':b15,'brti5':r5,'ask5':a5}
    return None
...
def add(row,side,f):
    ...
    pending.append({'ticker':row['ticker'],'side':side,'ts':row['ts'],'entry':ask,'max':bid,'min':bid,'x5':None,'x10':None,'ask5':None})
    r5='N/A' if f['brti5'] is None else ('%+.2f'%f['brti5'])
    print('LEAD_SHADOW CANDIDATE | %s | %s | ask %.3f | btc5 %+.2f | btc15 %+.2f | brti5 %s | ask5 %+.3f | left %.0fs'%(row['ticker'],side,ask,f['btc5'],f['btc15'],r5,f['ask5'],row['left']),flush=True)
...
def resolve(row):
    ...
    for e in pending:
        ...
        if ask is not None and ask-e['entry']>=.05 and e['ask5'] is None:e['ask5']=row['ts']
        if row['ts']-e['ts']>=HORIZON or row['left']<=0:done.append(e)
    for e in done:
        if e in pending:pending.remove(e)
        t5=None if e['x5'] is None else round(e['x5']-e['ts'],1); t10=None if e['x10'] is None else round(e['x10']-e['ts'],1); ar=None if e['ask5'] is None else round(e['ask5']-e['ts'],1)
        print('LEAD_SHADOW RESULT | %s | %s | entry %.3f | max_exec_gain %+.3f | adverse %+.3f | to_exec+5c %s | to_exec+10c %s | to_ask_reprice+5c %s'%(e['ticker'],e['side'],e['entry'],e['max']-e['entry'],e['min']-e['entry'],t5,t10,ar),flush=True)

print('SCALP LEAD SHADOW V2 START | 1s | broad research trigger | NO ORDERS',flush=True)
while True:
    t=time.time()
    try:
        row=snap()
        if row:
            ...
            resolve(row)
            for side in ('UP','DOWN'):
                f=candidate(row,side)
                if f:add(row,side,f)
            if int(row['ts'])%30==0:
                bv='N/A' if row['brti'] is None else ('%.2f'%row['brti'])
                print('LEAD_SHADOW HEARTBEAT | %s | %.2fm | BTC %.2f | BRTI %s | UP %.3f | DOWN %.3f | pending %d'%(row['ticker'],row['left']/60,row['btc'],bv,row['up_ask'],row['down_ask'],len(pending)),flush=True)
    except Exception as e:print('LEAD_SHADOW WARNING | %s: %s'%(type(e).__name__,e),flush=True)
