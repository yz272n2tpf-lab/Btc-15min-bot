...
OUT_GRID = Path("secondary_scalp_gapfiller_v1_1_grid.csv")
OUT_WF = Path("secondary_scalp_gapfiller_v1_1_walkforward_signals.csv")
OUT_HOLD = Path("secondary_scalp_gapfiller_v1_1_holdout_signals.csv")
...
print("="*80)
print("SECONDARY SCALP / PULLBACK GAP-FILLER — OFFLINE TOURNAMENT V1.1")
print("="*80)
...
# Pandas 3.x can drop grouping columns from groupby.apply results.
# Label groups explicitly and restore contract/side so downstream scoring
# remains version-independent.
_labeled_parts = []
for (_contract, _side), _grp in d.groupby(
    ["contract","side"], sort=False, dropna=False
):
    _x = label_group(_grp.copy())
    _x["contract"] = _contract
    _x["side"] = _side
    _labeled_parts.append(_x)

if not _labeled_parts:
    raise SystemExit("STOP: no candidate groups available after economic filters.")

d = pd.concat(_labeled_parts, ignore_index=True)
