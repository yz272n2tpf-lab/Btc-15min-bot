...
# === ROLLOVER PRE-DISCOVERY INTEGRATION SHADOW V1 ===
# Isolated research branch only. Signal-only / NO ORDERS.
_rollover_staged_market = None

def _prediscover_next_market(now):
    global _rollover_staged_market
    try:
        ...
        for m in data.get("markets",[]):
            ...
            if t.startswith("KXBTC15M") and op is not None and cl is not None and op>now and cl>op and (cl-op).total_seconds()==900 and op.second==0 and op.microsecond==0 and op.minute%15==0:
                candidates.append((op,cl,m))
        if candidates:
            op,cl,m=min(candidates,key=lambda x:x[0])
            if _rollover_staged_market is None:
                _rollover_staged_market={"ticker":str(m.get("ticker","")),"open":op,"close":cl,"market":m}
                print(f"ROLLOVER PRESTAGED | {_rollover_staged_market['ticker']} | opens {op.isoformat()} | NO ORDERS",flush=True)
    except Exception as exc:
        print(f"ROLLOVER PRESTAGE WAIT | {type(exc).__name__} | NO ORDERS",flush=True)

def _verified_staged_market(now):
    global _rollover_staged_market
    st=_rollover_staged_market
    if st is None or now<st["open"] or now>=st["close"]:
        if st is not None and now>=st["close"]: _rollover_staged_market=None
        return None
    # Research integration: exact staged metadata is eligible only at/after official open.
    # Existing target/quote/BRTI readiness downstream remains unchanged.
    return st["market"]
# === ROLLOVER PRE-DISCOVERY INTEGRATION SHADOW V1 END ===
