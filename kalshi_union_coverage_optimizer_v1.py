...
def main():
    print("="*78)
    print("UNION COVERAGE OPTIMIZER V1")
    ...
    scalp_results=[]
    total2=(len(scalp_ask)*len(scalp_fair)*len(scalp_edge)*len(scalp_min_tl)*
            len(scalp_max_tl)*len(scalp_gap)*len(profit_targets)*
            len(stop_losses)*len(horizons))
    print(f"Testing up to {total2:,} SCALP combinations on uncovered contracts...")

    # To keep runtime reasonable, prune clearly incompatible max/min time pairs.
            ...
            for sl in stop_losses:
             for hz in horizons:
              p=dict(
                  ...
                  profit_target=pt,stop_loss=sl,horizon=hz
              )
              s,_=score_scalp(valid_scalp_pool,candles,p)
              if s is not None:
                  scalp_results.append(s)
    ...
    lines=[]
    lines.append("="*78)
    lines.append("UNION COVERAGE OPTIMIZER V1 SUMMARY")
    lines.append("="*78)
    lines.append("")
