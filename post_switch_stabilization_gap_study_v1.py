...
def switch_stabilized_calls(frame, ask_max, fair_min, edge_min, gap_min, confirm_sec):
    picks=[]
    for ticker,g in frame.groupby("ticker"):
        g=g.sort_values("snapshot_utc").copy()
        if len(g)<2: continue

        sides=g["preferred_side_num"].astype(int)
        sw=(sides!=sides.shift(1))
        sw.iloc[0]=False
        switch_rows=g[sw]

        for _,s in switch_rows.iterrows():
            t0=s["snapshot_utc"]
            new_side=int(s["preferred_side_num"])
            ...
            ok=(
                int(c["preferred_side_num"])==new_side
            ...
            rr=c.copy()
            rr["switch_time"]=t0
            rr["switch_side"]=new_side
            rr["confirm_sec"]=confirm_sec
            rr["correct"]=int(c["preferred_side_num"])==int(c["final_side"])
            picks.append(rr)
            break
...
for a in asks:
  for f in fairs:
    for e in edges:
      for gp in gaps:
        for cs in confirms:
          q=switch_stabilized_calls(vg,a,f,e,gp,cs)
          if q.empty: continue
          rows.append({
...
print("="*78)
print("POST-SWITCH STABILIZATION GAP STUDY V1")
...
if res.empty:
    print("RESULT: No post-switch candidates fired on validation gaps.")
...
if eligible.empty:
    print("RESULT: No post-switch candidate cleared >=3 validation calls AND >=90% accuracy.")
    best=res.sort_values(["accuracy","calls","avg_ask"],ascending=[False,False,True]).head(6)
    print()
    print("Best validation-only candidates:")
    ...
    print()
    print("No holdout selection performed. Do not promote this branch.")
    print("No bot logic changed. No orders.")
    raise SystemExit(0)
...
print(
    f"Rule: after preferred-side switch, confirm {int(w.confirm_sec)}s later "
    f"same side with ask<={100*w.ask:.0f}c | fair>={100*w.fair:.0f}% | "
    f"edge>={100*w.edge:.0f}% | gap>=${w.gap:.0f}"
)

hq=switch_stabilized_calls(hg,w.ask,w.fair,w.edge,w.gap,int(w.confirm_sec))
if hq.empty:
