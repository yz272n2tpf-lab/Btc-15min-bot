"""Offline owner→HTTP→consumer→collector acceptance, no credentials/upstream."""
import os
import threading
import time
import unittest
...
from test_btc15_parity_measurement_regressions import load_functions as load_parity
...
class SourceIntegrity(unittest.TestCase):
    ...
    def test_http_consumers_and_real_collector_recover_sixty_publications(self):
        self.fetch.return_value=[point(int(self.clock*1000)-1000*i) for i in range(1,61)]
        self.owner.poll_once(force=True)
        ...
        try:
            with patch.dict(os.environ,BTC15_USE_SHARED_BRTI='1',BTC15_BRTI_TRANSPORT='legacy_shared_http',BTC15_BRTI_SHARED_URL=base):
                ...
                parity = load_parity()
                parity.kalshi_get = Mock(side_effect=AssertionError('upstream forbidden'))
                self.assertEqual(len(parity.direct_brti_payload()), 60)
                parity.kalshi_get.assert_not_called()
                self.fetch.assert_called_once()
                self.wall.return_value+=6
