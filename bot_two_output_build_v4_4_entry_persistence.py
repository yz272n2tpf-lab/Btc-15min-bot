...
try:
    ...
    _fair_split = int(len(_fair_contracts)*0.80)
    if _fair_split <= 0 or _fair_split >= len(_fair_contracts):
        raise RuntimeError('Not enough contracts for fair-value chronology split')

    _fair_model_contracts = _fair_contracts.iloc[:_fair_split]
    _fair_calib_contracts = _fair_contracts.iloc[_fair_split:]
...
# ============================================================
# V4.4 ENTRY PERSISTENCE STATE
# One focused change: require the same <=50c candidate side to
# persist across successive snapshots before promotion to a
# true OPPORTUNITY. FINAL logic remains untouched.
# ============================================================
_entry_state_path = Path("kalshi_entry_persistence_state.json")
_entry_state = {
    "contract": None,
    "side": None,
    "count": 0,
...
try:
    if _entry_state_path.exists():
        _entry_state = json.loads(_entry_state_path.read_text())
except Exception:
    pass
...
# Restore the entry rules from the prior live attractive-entry layer.
# V4.4 adds persistence confirmation without lowering any fair/edge thresholds.
_entry_persistence_count = 0
_entry_strengthening = False
...
if not _fair_ready:
    ...
    _opportunity_reason = 'Preferred side is above the <=50c entry ceiling'
else:
    _entry_candidate = bool(
        _opportunity_model_prob >= 0.60
        and _opportunity_edge >= 0.08
    )

    _entry_same_contract = (
        _entry_state.get("contract") == (_ladder_contract_ticker or '')
    )
    _entry_same_side = (
        _entry_state.get("side") == _opportunity_side
    )
    _entry_prev_fair = _entry_state.get("last_fair")
    _entry_prev_edge = _entry_state.get("last_edge")

    _entry_strengthening = bool(
        _entry_prev_fair is not None
        and _entry_prev_edge is not None
        and float(_opportunity_model_prob) >= float(_entry_prev_fair)
        and float(_opportunity_edge) >= float(_entry_prev_edge)
    )

    if _entry_candidate:
        if _entry_same_contract and _entry_same_side:
            _entry_persistence_count = int(_entry_state.get("count", 0)) + 1
        else:
            _entry_persistence_count = 1

        _entry_state = {
            "contract": (_ladder_contract_ticker or ''),
            "side": _opportunity_side,
            "count": _entry_persistence_count,
            "last_fair": float(_opportunity_model_prob),
            "last_edge": float(_opportunity_edge),
        }
        try:
            _entry_state_path.write_text(json.dumps(_entry_state))
        except Exception:
            pass
    else:
        _entry_persistence_count = 0
    ...
    _entry_high_quality_now = bool(
        _opportunity_model_prob >= 0.70
        and _opportunity_edge >= 0.15
    )

    if _entry_high_quality_now and _entry_persistence_count >= 2:
        _opportunity_status = 'OPPORTUNITY'
        _opportunity_reason = (
            'Persistent strong <=50c setup: same side 2+ snapshots, '
            'fair >=70% and edge >=15pt'
        )
    elif (
        _entry_candidate
        and _entry_persistence_count >= 3
        and _entry_strengthening
    ):
        _opportunity_status = 'OPPORTUNITY'
        _opportunity_reason = (
            'Persistent strengthening <=50c setup: same side 3+ snapshots'
        )
    elif _entry_candidate:
        _opportunity_status = 'VALUE WATCH'
        _opportunity_reason = (
            f'Cheap candidate building persistence '
            f'({_entry_persistence_count} snapshot(s))'
        )
    else:
        _opportunity_status = 'WATCH'
        _opportunity_reason = (
            'Price may be cheap, but target-aware evidence/edge '
...
print('ENTRY PERSISTENCE:', _entry_persistence_count)
print('ENTRY STRENGTHENING:', 'YES' if _entry_strengthening else 'NO')
...
#
# Gate:
#   - fair-value engine is live
#   - <=10 minutes remain (elapsed >=5)
#   - preferred fair probability >=92%
#   - BTC is at least $75 from the Kalshi target
#   - preferred side agrees with the actual side of the target
#
# The old strict final gate remains intact as an alternate path.
# ============================================================
...
try:
    ...
    _decisive_final_ready = bool(
        ...
        and _decisive_time_left <= 10.0
        and float(_fair_preferred) >= 0.92
        and _decisive_gap is not None
        and abs(_decisive_gap) >= 75.0
...
# Keep the original strict path. V4.3 adds only the decisive fair path.
if _decisive_final_ready:
    _final_15m_ready = True
    _final_15m_direction = _decisive_final_side
...
if _final_15m_ready:
    ...
    if _decisive_final_ready:
        ...
        _two_final_reason = (
            "Decisive target-aware fair gate: fair >=92%, "
            "<=10m left, >=$75 from Kalshi target"
        )
        _final_call_source = "DECISIVE FAIR"
    else:
        _two_final_confidence = float(current_confidence)
        _two_final_reason = (
            "Existing strict model gate + BRTI/Kalshi gate agree"
        )
        _final_call_source = "STRICT LEGACY"
else:
...
print("DECISIVE FAIR FINAL READY:", "YES" if _decisive_final_ready else "NO")
...
# ============================================================
_snapshot_log = Path('kalshi_two_output_live_log_v4_4.csv')
_snapshot_fields = [
    ...
    'opportunity_status','price_zone','entry_persistence','entry_strengthening',
    'ladder_status','ladder_direction','ladder_confidence','checkpoints',
    'expected_final_outcome','final_status','final_side','final_confidence','final_call_source','decisive_fair_ready',
...
_snapshot_row = {
    ...
    'entry_persistence': _entry_persistence_count,
    'entry_strengthening': bool(_entry_strengthening),
