#!/usr/bin/env python3
"""V3 research-only scalp lead-time collector. Signal-only. NO ORDERS.

Keeps the broad V2 trigger for recall, but adds a stricter QUALIFIED shadow
classification designed to test the actual project objective: BTC/BRTI impulse
must be present while Kalshi's contract price is still comparatively quiet.
"""
...
QUAL_BTC5=6.0; QUAL_BTC15=10.0; QUAL_BRTI5=4.0
QUAL_MAX_ASK5=.01; QUAL_MAX_ASK15=.03; QUAL_MIN_LEFT=45.0
...
def features(row,side):
    ...
    qualified=(
        broad and row['left']>=QUAL_MIN_LEFT
        and b5>=QUAL_BTC5 and b15>=QUAL_BTC15
        and (r5 is None or r5>=QUAL_BRTI5)
        and a5<=QUAL_MAX_ASK5 and a15<=QUAL_MAX_ASK15
        and accel>0
    )
    if not broad:return None
    return {'btc5':b5,'btc15':b15,'brti5':r5,'ask5':a5,'ask15':a15,'accel':accel,'qualified':qualified}

def add(row,side,f):
    k=(row['ticker'],side)
    if row['ts']-last.get(k,0)<20:return
    last[k]=row['ts']; p=side.lower(); ask=row[p+'_ask']; bid=row[p+'_bid']
    grade='QUALIFIED' if f['qualified'] else 'BROAD'
    pending.append({'ticker':row['ticker'],'side':side,'grade':grade,'ts':row['ts'],'entry':ask,'max':bid,'min':bid,'x5':None,'x10':None,'ask5_ts':None,'f':f.copy(),'left':row['left']})
    r5='N/A' if f['brti5'] is None else ('%+.2f'%f['brti5'])
    print('LEAD_V3 CANDIDATE | %s | %s | %s | ask %.3f | btc5 %+.2f | btc15 %+.2f | accel %+.2f | brti5 %s | ask5 %+.3f | ask15 %+.3f | left %.0fs'%(grade,row['ticker'],side,ask,f['btc5'],f['btc15'],f['accel'],r5,f['ask5'],f['ask15'],row['left']),flush=True)
...
def resolve(row):
    ...
    for e in done:
        ...
        win10=e['x10'] is not None
        print('LEAD_V3 RESULT | %s | %s | %s | entry %.3f | max_exec_gain %+.3f | adverse %+.3f | hit10 %s | to_exec+5c %s | to_exec+10c %s | kalshi_reprice+5c %s'%(e['grade'],e['ticker'],e['side'],e['entry'],e['max']-e['entry'],e['min']-e['entry'],win10,t5,t10,ar),flush=True)

print('SCALP LEAD SHADOW V3 START | broad + qualified parallel research | NO ORDERS',flush=True)
while True:
    t=time.time()
    try:
        row=snap()
        if row:
            ...
            if int(row['ts'])%30==0:
                ...
                print('LEAD_V3 HEARTBEAT | %s | %.2fm | BTC %.2f | BRTI %s | UP %.3f | DOWN %.3f | pending %d'%(row['ticker'],row['left']/60,row['btc'],bv,row['up_ask'],row['down_ask'],len(pending)),flush=True)
    except Exception as e:print('LEAD_V3 WARNING | %s: %s'%(type(e).__name__,e),flush=True)
