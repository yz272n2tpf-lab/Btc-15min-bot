...
from btc15_quote_proof_offpath_v1 import OffPathProofWriter, Witness
...
class Provider:
    def __init__(self):
        ...
        self.epoch = None
        self.close_ms = None
        # Candidate only: heavy proof serialization is isolated from consume().
        self.proof_writer = OffPathProofWriter(
            proof_path().parent / 'kalshi_quote_proofs_offpath_v1',
            self._validate_offpath_proof, MAX_BYTES)
    ...
    def consume(self, ticker, source_time, close_ms):
        with self.lock:
            ...
            try:
                ...
                return None
            # Freeze only a small causal identity plus a tuple of references while
            # holding the native lock. No JSON serialization or disk I/O occurs
            # before returning the quote. Evidence may be dropped if the bounded
            # worker is busy; strategy output must never wait for evidence.
            witness = Witness(
                ticker=ticker, source_time=source_time, epoch=self.epoch,
                consumed_ms=consumed, market_id=self.book.market_id,
                sid=self.book.sid, seq=self.book.seq,
                exchange_ts_ms=self.book.ts_ms, close_ms=close_ms)
            self.proof_writer.submit(witness, self.events)
            return quotes

    def _validate_offpath_proof(self, proof, witness):
        # Reconstruct independently on the worker. Any mutation/gap/restart or
        # cross-contract mismatch fails before publication.
        values, _ = replay(proof, witness.source_time, witness.ticker,
                           witness.close_ms, witness.consumed_ms)
        book = Book(witness.ticker)
        for event in proof['events']:
            book.apply(event)
        if tuple(values) != tuple(book.quotes(witness.consumed_ms, witness.close_ms)):
            raise ValueError('off-path quote mismatch')
        return [book.market_id, book.sid, book.seq, book.ts_ms]
