...
# ============================================================
_snapshot_log = Path('kalshi_two_output_live_log_v4_8_2.csv')
_snapshot_fields = [
...
print("\n" + "="*72)
print("V4.8.2 TRUE SCALP + EARLY-CONF SHADOW MODULE STARTING")
...
# =====================================================================
# V4.8.2 EARLY-CONFIDENCE SHADOW LOGGER
# =====================================================================
# Observation only. FINAL, Tier-1 ENTRY and true-scalp logic are unchanged.
# Research neighborhood:
#   ask <= 45c, fair >= 75%, edge >= 20%, 4-10m left, abs gap >= $25
# Logs sub-minute persistence/slope evidence that the 1m history cannot supply.
# =====================================================================

EARLY_CONF_LOG = Path("kalshi_early_conf_shadow_v1.csv")
EARLY_CONF_FIELDS = [
...
def log_early_conf_shadow(
    ...
    append_csv(EARLY_CONF_LOG, EARLY_CONF_FIELDS, {
...
while running:
    ...
    try:
        ...
        # Early-confidence shadow observation only.
        try:
            log_early_conf_shadow(
                now, now_ts, ticker, target, seconds_left, btc, btc_gap,
                str(_opportunity_side),
                float(_opportunity_ask),
                float(_opportunity_model_prob),
                float(_opportunity_edge),
            )
        except Exception:
            pass
