...
try:
    ...
    for _, _r in _fair_cal.iterrows():
        for _elapsed in range(1,15):
            _s = _fair_build_snapshot(
                ...
                final_side=int(_r['final_side']), elapsed=_elapsed,
            )
            if _s is not None:
                _s['ticker'] = _r['ticker']
                _s['start'] = _r['start']
    ...
    _fair_live = _fair_build_snapshot(
        _fair_btc, _fair_live_start, float(_audit_target), cut=_fair_now,
    )
...
# ============================================================
_snapshot_log = Path('kalshi_two_output_live_log_v4.csv')
_snapshot_fields = [
    ...
    'expected_final_outcome','final_status','final_side','final_confidence',
