...
# Fixed rescue rule from prior validation-selected tournament.
WINNER = dict(
    ask=.60, fair=.65, edge=.00,
    mn=2, mx=12, gap=0,
    pt=.08, sl=.05, hz=2
)

def pick_calls(frame, p):
    mask=(
    ...
    q=frame.loc[mask].sort_values(["ticker","snapshot_utc"])
    rows=[]
    for ticker,g in q.groupby("ticker"):
        for _,r in g.iterrows():
            o=future_outcome(r,p["pt"],p["sl"],p["hz"])
            if o is None:
                continue
            rr=r.copy()
            rr["win"]=int(o)
...
def side_switches_before(frame, ticker, ts):
    g=frame[(frame["ticker"]==ticker)&(frame["snapshot_utc"]<=ts)].sort_values("snapshot_utc")
    if g.empty:
        return 0
    s=g["preferred_side_num"].astype(int)
    return int((s!=s.shift(1)).sum()-1)

calls=pick_calls(hg, WINNER)

print("="*86)
print("RESCUE-SCALP HOLDOUT DIAGNOSTIC V1 — FIXED RULE / READ ONLY")
print("="*86)
print(f"Calls: {len(calls)} | Wins: {int(calls['win'].sum()) if len(calls) else 0} | Losses: {int((1-calls['win']).sum()) if len(calls) else 0}")
print("Rule is frozen from validation selection. Holdout is diagnosis only.")
print()

if calls.empty:
    raise SystemExit("No holdout calls under frozen rescue rule.")
...
for _,r in calls.iterrows():
    rows.append({
        ...
        "side_switches_before":side_switches_before(hg, str(r["ticker"]), pd.Timestamp(r["snapshot_utc"])),
        "preferred_side":str(r["preferred_side"]),
    })
...
for label,val in [("WINS",1),("LOSSES",0)]:
    ...
    print(
        ...
        f"avg gap ${g.gap_dollars.mean():.0f} | avg range ratio {g.range_ratio.mean():.2f} | "
        f"avg prior switches {g.side_switches_before.mean():.2f}"
    )
    for _,x in g.iterrows():
        print(
            f"{x.ticker} | ask {x.ask_c:.0f}c | fair {x.fair_pct:.0f}% | edge {x.edge_pct:.0f}% | "
            f"{x.minutes_left:.1f}m left | gap ${x.gap_dollars:.0f} | ratio {x.range_ratio:.2f} | "
            f"switches {int(x.side_switches_before)} | {x.preferred_side}"
        )
    print()

print("SEPARATION CHECKS — DIAGNOSTIC ONLY, NOT RULE SELECTION")
print("-"*86)
for col in ["ask_c","fair_pct","edge_pct","minutes_left","gap_dollars","range_ratio","side_switches_before"]:
    w=out.loc[out.win==1,col]
    l=out.loc[out.win==0,col]
    if len(w) and len(l):
        print(f"{col:22s} wins avg {w.mean():8.2f} | losses avg {l.mean():8.2f} | diff {w.mean()-l.mean():+8.2f}")

print()
print("INTEGRITY: no retuning on holdout; no new threshold chosen; no bot logic changed; no orders.")
print("="*86)

out.to_csv("rescue_scalp_holdout_diagnostic_v1.csv", index=False)
