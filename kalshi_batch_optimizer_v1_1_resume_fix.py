...
def final_tournament(hist, model_c, calib_c, valid_c, holdout_c):
    ...
    # RESUME MODE:
    # The first V1 run can safely be resumed if it already completed the
    # 7,776-combination FINAL tournament but failed later in ENTRY analysis.
    # We re-train the model and recompute the untouched holdout, but reuse
    # the already-ranked validation tournament instead of sweeping all gates
    # again. This keeps the holdout honest while avoiding wasted compute.
    resume_requested = "--resume" in sys.argv
    if resume_requested and FINAL_RESULTS.exists():
        prev = pd.read_csv(FINAL_RESULTS)
        param_cols = [
            "fair_min","max_time_left","early_boundary",
            "early_gap","late_gap","min_support","min_dist_range"
        ]
        if not prev.empty and set(param_cols).issubset(prev.columns):
            if "validation_rank" in prev.columns:
                prev = prev.sort_values("validation_rank")
            else:
                prev = prev.reset_index(drop=True)
            winner_params = {k: prev.iloc[0][k] for k in param_cols}
            holdout_score = score_gate(holdout, winner_params)
            print(
                "RESUME: reused completed FINAL validation tournament; "
                "recomputed winner on untouched holdout only."
            )
            return prev, winner_params, holdout_score, valid, holdout
        print(
            "RESUME requested, but prior FINAL results are incomplete; "
...
def entry_tournament(cal):
    ...
    truth = (
        ...
        .rename(
            columns={
                "ticker":"contract",
                "final_side":"official_final_side",
            }
        )
    )
    # Live logs may already contain their own final_side field. Keep that
    # diagnostic field untouched and merge official settlement truth under
    # a unique name so pandas cannot create final_side_x/final_side_y.
    ...
    settled = d[d["official_final_side"].notna()].copy()
                ...
                for persistence in persistence_grid:
                    ...
                    if calls:
                        ...
                        correct = (
                            ...
                            == c["official_final_side"].astype(int)
                        )
                        rows.append({
                            "ask_max":ask_max,
                            "fair_min":fair_min,
...
def main():
    ...
    if "--resume" in sys.argv:
        print("MODE: RESUME — reuse completed FINAL tournament if available")
    else:
        print("MODE: FULL — run complete tournament")

    # Cheap preflight FIRST: catch live-log schema/merge issues before
    # spending time on historical model work.
    if not CAL.exists():
        raise SystemExit(f"MISSING REQUIRED FILE: {CAL}")
    _pre_cal = pd.read_csv(CAL)
    _pre_logs = load_live_logs()
    if not _pre_logs.empty:
        _pre_truth = (
            _pre_cal[["ticker","result"]]
            .drop_duplicates("ticker", keep="last")
            .rename(columns={"ticker":"contract","result":"official_result"})
        )
        _pre_merge = _pre_logs.merge(_pre_truth, on="contract", how="left")
        print(
            "PREFLIGHT: live-log schema + official-settlement merge PASS "
            f"({_pre_logs['contract'].nunique()} logged contracts)"
        )
    else:
        print("PREFLIGHT: no compatible live logs yet; FINAL tournament can still run.")
