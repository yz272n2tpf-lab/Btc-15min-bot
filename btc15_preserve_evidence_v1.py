...
class _SizeOnly:
    """Count a streamed ZIP without retaining data or touching the volume."""
    def __init__(self): self.size = 0
    def write(self, data): self.size += len(data); return len(data)
    def tell(self): return self.size
    def flush(self): pass
    def seek(self, *args): raise OSError('non-seekable sizing stream')


def _write_archive(destination, files, sizes, created):
    records = []
    with zipfile.ZipFile(destination, 'w', compression=zipfile.ZIP_DEFLATED, allowZip64=True) as archive:
        ...
        archive.writestr('manifest.json', json.dumps(dict(
            created_utc=created, files=records,
...
def preserve(root, label):
    ...
    # ZIP compression is streamed twice only when a worst-case bound does not fit.
    # The first pass writes no bytes and verifies the exact prefix hashes. Its
    # non-seekable ZIP descriptors make its size a conservative bound for pass2.
    expected = None
    needed = total * 1.02
    if shutil.disk_usage(root).free < needed + 10_000_000:
        counter = _SizeOnly()
        expected = _write_archive(counter, files, sizes, created)
        needed = counter.size + 100_000
    if shutil.disk_usage(root).free < needed + 10_000_000:
        raise RuntimeError('insufficient compressed archive headroom; original evidence untouched')
    with temporary.open('xb') as stream:
        records = _write_archive(stream, files, sizes, created)
        stream.flush()
        import os
        os.fsync(stream.fileno())
    if expected is not None and records != expected:
        raise RuntimeError('dataset prefix changed between sizing and archive; originals retained')
