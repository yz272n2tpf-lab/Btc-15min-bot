...
class T(unittest.TestCase):
 def test_unchanged_source_skips_expensive_reanalysis(self):
  ...
  with patch.object(v.forward,"fetch_csv_rows",return_value=([{"x":1}],"same")), patch.object(v.time,"time",return_value=1001.0), patch.object(v.forward,"build_summary",side_effect=AssertionError("must skip")), patch.object(v,"summarize",side_effect=AssertionError("must skip")):
   out=v.cycle()
