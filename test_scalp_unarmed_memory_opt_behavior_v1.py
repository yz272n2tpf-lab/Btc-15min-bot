...
","same")), patch.object(v.forward,"parse_csv_raw",side_effect=AssertionError("must not parse unchanged source")), patch.object(v.time,"time",return_value=1001.0), patch.object(v.forward,"build_summary",side_effect=AssertionError("must skip")), patch.object(v,"summarize",side_effect=AssertionError("must skip")):
 ...
 def test_live_state_drops_heavy_evidence_lists(self):
  heavy={k:[{"blob":"x"*1000} for _ in range(10)] for k in ("ended_unarmed_records","armed_no_validated_exit_records","recovered_handoffs","prearm_reset_failed_primary_candidate_ids","blueprint_review_items")}
  fake={"ok":True,"orders":False,"baseline_completed_serial_opportunities":1,"projected_completed_serial_opportunities":1,"projected_additional_serial_opportunities":0,"ended_unarmed_n":10,"armed_no_validated_exit_n":10,"armed_no_validated_exit_preserved_blocking":True,"armed_no_validated_exit_reclassified_as_ended_unarmed_n":0,"post_unarmed_later_completed_n":0,"post_unarmed_later_plus10_n":0,"completed_meaningful_10c_candidates":0,"selected_meaningful_10c":0,"blocked_prearm_meaningful_10c":0,"true_post_exit_missed_meaningful_10c":0,"lifecycle_reset_review_ready":True,"tightening_development_nominee_btc30_min":None,"tightening_holdout_review_ready":False,"tightening_holdout_supports_nominee":False,"tightening_development_baseline":{},"tightening_development_nominee":None,"tightening_holdout_baseline":{},"tightening_holdout_nominee":None,"tightening_decision":"NO_DEVELOPMENT_NOMINEE","prearm_reset_completed_primary_n":0,"prearm_reset_failed_primary_n":0,"prearm_reset_grid":{},"protection_audit":{},"forward_audit":{},"blueprint_review_blockers":[],**heavy}
  v._LAST_SOURCE_SHA="old";v._LAST_ANALYSIS_AT=0
  with patch.object(v.forward,"fetch_csv_raw",return_value=(b"x","sha2")),patch.object(v.forward,"parse_csv_raw",return_value=[]),patch.object(v.time,"time",return_value=3000),patch.object(v.forward,"poll_timer_status"),patch.object(v.forward,"build_summary",return_value={}),patch.object(v,"summarize",return_value=fake):v.cycle()
  for k in heavy:self.assertNotIn(k,v.STATE)
  self.assertEqual(v.STATE["ended_unarmed_n"],10);self.assertFalse(v.STATE["orders"])
