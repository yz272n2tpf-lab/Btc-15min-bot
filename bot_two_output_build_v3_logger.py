...
# ============================================================
# TWO-OUTPUT LIVE DECISION VIEW
# 1) EARLY OPPORTUNITY = informational, never a final call.
# 2) FINAL OUTCOME = PASS unless the EXISTING strict final gate is ready.
# This block changes presentation only. It does not change model math,
# validated ladder thresholds, scalp logic, or place orders.
# ============================================================

# Safely identify the live Kalshi ask for the current model side.
...
if _model_15m_direction == "UP":
    _opportunity_ask = _two_up_ask
else:
    _opportunity_ask = _two_down_ask

# current_confidence is the model probability on its preferred/raw side.
_opportunity_model_prob = float(current_confidence)
_opportunity_edge = (
    _opportunity_model_prob - _opportunity_ask
    if _opportunity_ask is not None
    else None
)

# Price-zone guidance only; it is NOT a validated final-outcome rule.
if _opportunity_ask is None:
    _opportunity_price_zone = "NO LIVE ASK"
elif _opportunity_ask <= 0.30:
    _opportunity_price_zone = "DEEP VALUE PRICE"
elif _opportunity_ask <= 0.40:
    _opportunity_price_zone = "ATTRACTIVE PRICE"
elif _opportunity_ask <= 0.50:
    _opportunity_price_zone = "ACCEPTABLE PRICE"
else:
    _opportunity_price_zone = "EXPENSIVE / WAIT"

# Do not promote an unqualified RAW FORECAST as an actionable opportunity.
# Reuse the already-validated same-contract ladder instead of inventing
# another confidence threshold here.
if _ladder_status == "RAW FORECAST":
    _opportunity_status = "WATCH"
    _opportunity_reason = (
        "Raw forecast only — wait for validated same-contract qualification"
    )
elif _opportunity_ask is None:
    _opportunity_status = "WATCH"
    _opportunity_reason = "Waiting for live Kalshi ask"
elif _opportunity_ask > 0.50:
    _opportunity_status = "WAIT"
    _opportunity_reason = "Qualified side, but Kalshi ask is above 50 cents"
elif _opportunity_edge is None or _opportunity_edge <= 0:
    _opportunity_status = "WAIT"
    _opportunity_reason = "Qualified side, but no positive model edge versus live ask"
else:
    _opportunity_status = "OPPORTUNITY"
    _opportunity_reason = (
        "Validated same-contract qualification + positive edge at <=50 cents; "
        "NOT a final call"
    )

print("\n============================================================")
print("EARLY OPPORTUNITY — NOT THE FINAL OUTCOME")
print("============================================================")
print("CONTRACT:", _ladder_contract_ticker or "UNKNOWN")
print("EARLY BIAS:", _model_15m_direction)
print("RAW MODEL CONFIDENCE:", f"{float(current_confidence):.1%}")
print(
    "LIVE KALSHI ASK:",
    f"{_opportunity_ask:.0%}" if _opportunity_ask is not None else "UNKNOWN",
)
print(
    "MODEL EDGE VS ASK:",
    f"{_opportunity_edge:+.1%}" if _opportunity_edge is not None else "UNKNOWN",
)
print("PRICE ZONE:", _opportunity_price_zone)
print("OPPORTUNITY STATUS:", _opportunity_status)
print("REASON:", _opportunity_reason)
print("FINAL-CALL AUTHORITY: NO — this section is opportunity guidance only")
...
# ============================================================
# AUTOMATIC TWO-OUTPUT LIVE SNAPSHOT LOG
# Appends one row every time bot.py runs. The 30-second outer loop
# therefore creates a continuous contract-by-contract audit trail.
# Research/signal-only logging only; no model thresholds are changed.
# ============================================================
_snapshot_log = Path("kalshi_two_output_live_log.csv")
_snapshot_fields = [
    "timestamp_utc",
    "contract",
    "elapsed_min",
    ...
    "confidence_change",
    "strict_model_ready",
    "brti_kalshi_gate_ready",
]

try:
    _snapshot_btc_price = (
        float(data.iloc[-1]["Close"])
        if "data" in globals() and hasattr(data, "empty") and not data.empty
        else None
    )
except Exception:
...
_snapshot_gap_dollars = (
    ...
    if _snapshot_btc_price is not None and _snapshot_target is not None
    else None
)

try:
    _snapshot_checkpoints = ",".join(
        sorted(
            _ladder_state.get("checkpoints", {}).keys(),
            key=lambda x: int(x),
        )
...
_snapshot_row = {
    ...
    "early_bias": _model_15m_direction,
    "raw_model_confidence": float(current_confidence),
    "preferred_kalshi_ask": _opportunity_ask,
    "model_edge_vs_ask": _opportunity_edge,
