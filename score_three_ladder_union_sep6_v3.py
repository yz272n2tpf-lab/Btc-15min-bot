...
for r in srows:
    ...
    # Contract membership is the authoritative cohort filter here.
    # The forward scalp event log can span beyond the fixed scorer clock window
    # even when the contract itself belongs to the 56-contract Sep-6 cohort.
    scalp_signal_rows+=1
    if c not in S:
        ml=num(r.get("minutes_left"))
