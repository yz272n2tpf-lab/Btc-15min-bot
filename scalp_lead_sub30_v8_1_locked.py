#!/usr/bin/env python3
"""LOCKED V8.1 surgical sub-30 scalp shadow. SIGNAL ONLY. NO ORDERS."""
...
def add81(row,side,f,label,route):
...
print('SCALP V8.1 START | LOCKED SURGICAL SUB30 | 30-45 PROTECTED | 7-15 REJECTED | NO ORDERS',flush=True)
while True:
    t=time.time()
    try:
        row=snap()
        if row:
            ...
            for side in ('UP','DOWN'):
                ...
                if confirmed81(row,side,route,ok):add81(row,side,f,label,route)
