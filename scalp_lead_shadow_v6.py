...
score_total = defaultdict(lambda: {'n':0,'hit5':0,'hit10':0,'fast10':0,'gain_sum':0.0,'adverse_sum':0.0})
score_contract = defaultdict(lambda: {'n':0,'hit5':0,'hit10':0,'fast10':0,'gain_sum':0.0,'adverse_sum':0.0})
...
def _bump(bucket, e, gain, adverse, t5, t10):
    s = bucket[e['grade']]
    s['n'] += 1
...
def score_result(e, gain, adverse, t5, t10):
    _bump(score_total, e, gain, adverse, t5, t10)
    _bump(score_contract, e, gain, adverse, t5, t10)
...
def _fmt_score(s):
    if not s['n']:
        return 'n=0'
    n=s['n']
    return 'n=%d hit5=%.1f%% hit10=%.1f%% fast10=%.1f%% avgGain=%+.3f avgAdv=%+.3f' % (
        n,100*s['hit5']/n,100*s['hit10']/n,100*s['fast10']/n,s['gain_sum']/n,s['adverse_sum']/n)
...
print('SCALP LEAD SHADOW V6 START | V5 baseline + price-zone/data-quality filter + streamlined autoscore | NO ORDERS', flush=True)
while True:
    t = time.time()
    try:
        row = snap()
